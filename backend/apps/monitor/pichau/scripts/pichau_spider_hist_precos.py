...
class PichauHistPrecos:
    ...
    def requisicao(self, url: str):
        ...
        try:
            ...
            stock_available = page_content.find("meta", attrs={"property": "product:availability"})
            if stock_available is not None:
                stock_available = stock_available.get("content")

                if stock_available == "instock":
                    # Pega o elemento que contém o preço
                    product_price = page_content.find("meta", attrs={"property": "product:price:amount"})
                    # Pega o preço
                    product_price = product_price.get("content")
                    # Formata ele para ser R$ xx,xx
                    product_price = product_price.split("R$")
                    product_price = "R$ " + product_price[1]
            # Senão conseguiu verificar se o produto tem estoque,
            # logo ele também não conseguirá pegar o preço. E deve
            # avisar que houve falha ao acessar a página
            else:
    ...
    def scraping(self):
        ...
        lista_produtos = ProdutoEletrInf.objects.filter(plataforma="pichau_novidades", monitorar=True)
