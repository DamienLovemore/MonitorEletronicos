...
from apps.helpers.settings_vars import url_kabum_ofertas, webhook_kabum_ofertas_ate_20, webhook_kabum_ofertas_ate_40, \
    webhook_kabum_ofertas_ate_60, webhook_kabum_ofertas_ate_100
from apps.monitor.integrations.discord.notifier import DiscordNotify


# -----------------------------------------------------------------
# Arquivo responsável por olhar por ofertas nos produtos da kabum,
# nas categorias de gamer, smartphones/celulares, hardware e
# computadores
# -----------------------------------------------------------------
...
class KabumOfertas:
    ...
    def requisicao(self, url: str):
        try:
            ...
            # Pega todos os produtos que estão com ofertas
            offers = page_content.find_all("div", attrs={"class": "productCard"})
    ...
    def criar_modelo(self, oferta: dict):
        ...
        try:
            ...
            modelo_produto, criou = ProdutoEletrInf.objects.get_or_create(nome=oferta["nome"], imagem=oferta["imagem"],
            ...
            modelos_produtos_duplicados = ProdutoEletrInf.objects.filter(nome=oferta["nome"], imagem=oferta["imagem"],
                                                                          url_produto=oferta["url_produto"],
                                                                          perc_desconto=oferta["perc_desconto"],
                                                                          plataforma=oferta["plataforma"])
    ...
    def notificar_ofertas_discord(self, itens_notificar: list):
        ...
        for item_notificar in itens_notificar:
            ...
            if (item_notificar["perc_desconto"] >= 2) and (item_notificar["perc_desconto"] <= 20):
                url_webhook_channel = webhook_kabum_ofertas_ate_20
            elif (item_notificar["perc_desconto"] >= 21) and (item_notificar["perc_desconto"] <= 40):
                url_webhook_channel = webhook_kabum_ofertas_ate_40
            elif (item_notificar["perc_desconto"] >= 41) and (item_notificar["perc_desconto"] <= 60):
                url_webhook_channel = webhook_kabum_ofertas_ate_60
            elif item_notificar["perc_desconto"] > 60:
                url_webhook_channel = webhook_kabum_ofertas_ate_100

            # Cria a classe responsável por mandar a mensagem,
            # passando o conteúdo a ser notificado,
            notificador_discord = DiscordNotify(item_notificar, url_webhook_channel, self.tipo_notificacao, self.loja)
    ...
    def scraping(self, page: int):
        ...
        try:
            ...
            for oferta in todas_ofertas:
                nome = oferta.find("h2", attrs={"class": "nameCard"}).next_element

                imagem = oferta.find("img", attrs={"class": "imageCard"})
                imagem = imagem.get("src")

                url_produto = oferta.find("a").get("href")
                # Completa o resto da url, pois no href pode se omitir
                # a página inicial se tiver pegando recursos dela mesma
                url_produto = "https://www.kabum.com.br" + url_produto

                preco = oferta.find("span", attrs={"class": "priceCard"}).next_element

                perc_desconto = oferta.find("div", attrs={"class": "discountTagCard"})
                perc_desconto = float(perc_desconto.find("p").next_element)

                estoque_disponivel = oferta.find("div", attrs={"class": "remainingTagCard"})
                estoque_disponivel = estoque_disponivel.find("span").next_element
    ...
    def get_items(self):
        ...
        # (Olha a primeira página, que sempre tem as
